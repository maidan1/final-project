      ...
      if (axios.isAxiosError(err)) {
        if (err.response) {
          ...
          if (err.response.status === 401) {
            setErrorsState({ email: "Invalid email or password" });
          } else {
            setErrorsState({ general: "Server error, please try again later" });
          }
        } else if (err.request) {
          setErrorsState({
            general: "No response from the server, please try again",
          });
        } else {
